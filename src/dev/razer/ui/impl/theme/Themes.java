...
public enum Themes {
    ...
    public Color getAccentColor(Vector2d screenCoordinates) {
        ...
        if (this.triColor) {
            ...
            if (blendFactor <= 0.5) return ColorManager.mixColors(getSecondColor(), getFirstColor(), blendFactor * 2D);
                // Blend between second and third color
            else return ColorManager.mixColors(getThirdColor(), getSecondColor(), (blendFactor - 0.5) * 2D);
        }

        // Two color blending
        return ColorManager.mixColors(getFirstColor(), getSecondColor(), getBlendFactor(screenCoordinates));
    }
